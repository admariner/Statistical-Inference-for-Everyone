...
class MCMCModel_Meta(object):
    ...
    def plot_parameters(self,*args):
        if not args:
            args=self.keys

        estimates=self.best_estimates()
        keys=args

        y=np.arange(len(keys))
        x=np.array([estimates[key][1] for key in keys])
        xl=np.array([estimates[key][0] for key in keys])
        xu=np.array([estimates[key][2] for key in keys])        

        py.errorbar(x,y,xerr=np.vstack([x-xl,xu-x]),fmt='o',lw=1)
        py.gca().set_yticks(y)
        py.gca().set_yticklabels(keys)        
